...
resource "kubectl_manifest" "azdevops_postgres_agent" {
  for_each  = (var.ado-agents_config.enable) ? { for agent in var.ado-agents_config.postgres_agents : agent.agent_name => agent } : {}
  yaml_body = <<YAML
---
apiVersion: keda.sh/v1alpha1
kind: ScaledJob
